...
function Receipts(){
 ...
 return <div className="app-page receipts-page">
  ...
  {exporting&&<p role="status">Готовим полную историю за выбранный период…</p>}
   {!busy&&!error&&!rows.length?<section className="surface history-empty"><img className="history-empty-art" src={search||category!=='all'||scope!=='all'?'/assets/visual-kit-v1/empty-blank.webp':'/assets/visual-kit-v1/empty-create.webp'} alt="" aria-hidden="true" loading="lazy"/><h2>{search||category!=='all'||scope!=='all'?'Ничего не найдено':'Здесь будет история расходов'}</h2><p>{search||category!=='all'?'Попробуйте другие условия поиска.':'Добавьте покупку: достаточно суммы. Остальное можно указать позже.'}</p>{!search&&category==='all'&&scope==='all'?<button className="primary-action history-empty-action" onClick={()=>setAdding(true)}><Plus size={17}/>Добавить первый расход</button>:null}</section>:<div className="history-list" aria-busy={busy}>{rows.map((r,i)=>{
