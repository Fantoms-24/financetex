...
import { dateRu, greeting, money } from '~/lib/format'
...
export function Overview() {
  ...
  const daily = Math.max(0, Math.floor(left / daysLeft))
  ...
  const unpaid = boot.bills
    .filter((bill) => !bill.paid_cycle)
    .sort((a, b) => {
      const until = (day: number) => day >= now.getDate() ? day - now.getDate() : day - now.getDate() + days
      return until(a.day_of_month) - until(b.day_of_month)
    })
  const nextBill = unpaid[0]
  const focus = budget <= 0
    ...
    : left < 0
      ...
      : nextBill
        ? { title: `Ближайший платёж — ${nextBill.title}`, body: `${nextBill.day_of_month}-го числа · ${money(nextBill.amount)}`, action: 'Открыть план', to: '/bills' }
