...
export const scanReceipt = createServerFn({ method: 'POST' })
  .validator((d: { image: string }) => ({ image: String(d.image || '') }))
  .handler(async ({ data }) =>
    guarded(async (user) => {
      const { baseUrl, apiKey, model } = await getLlmConfig()
      if (!apiKey) {
        return { error: 'Админ ещё не вставил ключ. Скан недоступен' } as const
      }
      if (!data.image.startsWith('data:image/')) {
        return { error: 'Не получилось прочитать фото' } as const
      }
...
- Если что-то не читается, всё равно верни JSON с тем, что удалось понять.`
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          return { error: `Сервис распознавания не ответил (${resp.status})` } as const
        }
        const json: any = await resp.json()
        content = json?.choices?.[0]?.message?.content || ''
      } catch (e: any) {
        return { error: 'Сервис распознавания недоступен. Попробуйте ещё раз' } as const
      }

      const parsed = extractJson(content)
      if (!parsed) return { error: 'Не разобрал чек. Попробуйте ещё раз или впишите вручную' } as const

      const items: Array<{ name: string; qty: number | null; price: number; category: string }> = Array.isArray(
        parsed.items,
      )
        ? parsed.items
            .map((i: any) => ({
      ...
      const category = asCategory(
        parsed.category ||
          (Object.keys(dominant).length
            ? Object.entries(dominant).sort((a, b) => b[1] - a[1])[0][0]
            : 'other'),
      )
