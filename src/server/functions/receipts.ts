...
import type { Receipt, ReceiptItem } from './bootstrap'

export const addReceipt = createServerFn({ method: 'POST' })
  .validator((d: { store: string; total: number; note?: string; category?: string; purchased_at?: string }) => ({
...
export const getReceipt = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) =>
    guarded(async (user) => {
      const r = await q1<Receipt>(
        `SELECT id, store, purchased_at, total, category, verdict, note, image, created_at
           FROM receipts WHERE id = $1 AND user_id = $2`,
        [data.id, user.id],
      )
      if (!r) return { receipt: null, items: [] as Array<ReceiptItem> }
      const items = await q<ReceiptItem>(
        `SELECT id, name, qty, price, category FROM receipt_items WHERE receipt_id = $1`,
        [data.id],
      )
      return {
        receipt: { ...r, purchased_at: r.purchased_at ? String(r.purchased_at).slice(0, 10) : null, total: Number(r.total) },
        items: (items ?? []).map((i) => ({ ...i, price: Number(i.price), qty: i.qty == null ? null : Number(i.qty) })),
      }
    }),
  )

export const listReceipts = createServerFn({ method: 'GET' })
  .validator((d?: { limit?: number }) => ({ limit: Math.min(Number(d?.limit || 120), 300) }))
  .handler(async ({ data }) =>
    guarded(async (user) => {
      const rows = await q<Receipt>(
        `SELECT id, store, purchased_at, total, category, verdict, note, image, created_at
           FROM receipts
          WHERE user_id = $1
